"""
Module gérant les séquences de Quick Time Event (QTE).
"""
import random
import time
import os
...
class QTE:
    ...
    def __init__(self, game, nb_tours, min_inputs, max_inputs,
                 temps_reaction, pool_lettres="AZERTY"):
        self.game = game
        ...
        # Attributs GUI initialisés à None
        self.root = None
    ...
    def _get_key_timed(self, timeout):
        """Récupère une touche avec un temps limite."""
    ...
    def start_console(self):
        ...
        for i in range(self.nb_tours):
            ...
            for lettre_attendue in sequence:
                key_pressed, _ = self._get_key_timed(self.temps_reaction)

                if key_pressed is None:
                    print("\n\n⚠️  TROP LENT ! La prise vous échappe !")
                    round_success = False
                    break
                if key_pressed != lettre_attendue:
                    print(f"\n\n❌  ERREUR ! Vous glissez "
                          f"({key_pressed} au lieu de {lettre_attendue}) !")
                    round_success = False
                    break
    ...
    def start_gui(self):
        ...
        self.lbl_timer_title = tk.Label(self.top, text="TEMPS LIMITE",
                                        font=("Helvetica", 12, "bold"), fg="#f55", bg="#222")
        self.lbl_timer_title.pack(pady=(20, 5))

        self.progress_bar = ttk.Progressbar(self.top, variable=self.progress_var,
                                            maximum=100, length=800, style="TProgressbar")
        self.progress_bar.pack(pady=(0, 20))

        self.lbl_info = tk.Label(self.top, text="PRÉPAREZ-VOUS...",
                                 font=("Helvetica", 20), fg="white", bg="#222")
        ...
        self.lbl_status = tk.Label(self.top, text="", font=("Helvetica", 14),
                                   fg="#f55", bg="#222")
    ...
    def on_gui_key(self, event):
        ...
        if char_pressed == expected:
            ...
            self.lbl_status.config(
                text=f"ERREUR ! ({char_pressed} au lieu de {expected})",
                fg="red"
            )
            self.gui_fail()
